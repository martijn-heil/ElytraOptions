...
public class ToggleSoundSubCmd extends NinSubCommandExecutor
{
    ...
    public void execute(CommandSender commandSender, String[] args) throws ValidationException, TechnicalException
    {
        ...
        else if (args != null && args.length == 1 && !(args[0].equalsIgnoreCase("on") || args[0].equalsIgnoreCase("off"))) // target player is specified, value is not.
        {
            ...
            // Toggle it.
            ep.setUsingSound(!ep.isUsingSound());

            ep.toNinOnlinePlayer().sendMessage(MessageColor.INFO, String.format("%s turned your boost sound %s.", commandSender.getName(), BooleanUtils.toStringOnOff(ep.isUsingSound())), ElytraOptions.getInstance());
        }
        else if (args != null && args.length == 1 && (args[0].equalsIgnoreCase("on") || args[0].equalsIgnoreCase("off"))) // target player is not specified, value is.
        {
            ...
            ep.toNinOnlinePlayer().sendMessage(MessageColor.INFO, String.format("Boost sound is now turned %s", BooleanUtils.toStringOnOff(newValue)), ElytraOptions.getInstance());
        }
        else if(args == null || args.length <= 0) // no arguments specified.
        {
            ...
            ep.toNinOnlinePlayer().sendMessage(MessageColor.INFO, String.format("Boost sound is now turned %s", BooleanUtils.toStringOnOff(ep.isUsingSound())), ElytraOptions.getInstance());
        }
    }
}
