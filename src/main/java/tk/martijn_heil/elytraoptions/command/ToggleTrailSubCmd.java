...
public class ToggleTrailSubCmd extends NinSubCommandExecutor
{
    @Override
    public void execute(CommandSender commandSender, String[] args) throws ValidationException, TechnicalException
    {
        ...
        {
            if(!commandSender.hasPermission("elytraoptions.flight.trail.others")) throw new AccessDeniedException(commandSender);
            ...
            // toggle it
            ep.setUsingTrail(!ep.isUsingTrail());

            ep.toNinOnlinePlayer().sendMessage(MessageColor.INFO, String.format("%s turned your flight trail %s.", commandSender.getName(), BooleanUtils.toStringOnOff(!ep.isUsingTrail())), ElytraOptions.getInstance());
        }
        ...
        else if (args != null && args.length == 1 && (args[0].equalsIgnoreCase("on") || args[0].equalsIgnoreCase("off"))) // target player is not specified, value is.
        {
            ...
            if(!commandSender.hasPermission("elytraoptions.flight.trail")) throw new AccessDeniedException(commandSender);

            EPlayer ep = ElytraOptions.getEPlayer((Player) commandSender);
            if(ep == null) throw new PlayerNotFoundException(commandSender);

            boolean newValue = (args[0].equalsIgnoreCase("on"));
            ep.setUsingTrail(newValue);

            ep.toNinOnlinePlayer().sendMessage(MessageColor.INFO, String.format("Flight trail is now turned %s", BooleanUtils.toStringOnOff(newValue)), ElytraOptions.getInstance());
        }
        else if(args == null || args.length <= 0) // no arguments specified.
        {
            ...
            ep.toNinOnlinePlayer().sendMessage(MessageColor.INFO, String.format("Flight trail is now turned %s", BooleanUtils.toStringOnOff(ep.isUsingTrail())), ElytraOptions.getInstance());
        }
    }
}
