...
public class ToggleBoostSubCmd extends NinSubCommandExecutor
{
    ...
    public void execute(CommandSender commandSender, String[] args) throws ValidationException, TechnicalException
    {
        ...
        {
            if(!commandSender.hasPermission("elytraoptions.flight.boost.others")) throw new AccessDeniedException(commandSender);
            ...
            // toggle it
            ep.setUsingBoost(newValue);

            ep.toNinOnlinePlayer().sendMessage(MessageColor.INFO, String.format("%s turned your flight boosting %s.", commandSender.getName(), BooleanUtils.toStringOnOff(newValue)), ElytraOptions.getInstance());
        }
        else if (args != null && args.length == 1 && !(args[0].equalsIgnoreCase("on") || args[0].equalsIgnoreCase("off"))) // target player is specified, value is not.
        {
            ...
            // Toggle it.
            ep.setUsingBoost(!ep.isUsingBoost());

            ep.toNinOnlinePlayer().sendMessage(MessageColor.INFO, String.format("%s turned your flight boosting %s.", commandSender.getName(), BooleanUtils.toStringOnOff(ep.isUsingBoost())), ElytraOptions.getInstance());
        }
        else if (args != null && args.length == 1 && (args[0].equalsIgnoreCase("on") || args[0].equalsIgnoreCase("off"))) // target player is not specified, value is.
        {
            ...
            if(!commandSender.hasPermission("elytraoptions.flight.boost")) throw new AccessDeniedException(commandSender);
            ...
            ep.toNinOnlinePlayer().sendMessage(MessageColor.INFO, String.format("Flight boosting is now turned %s", BooleanUtils.toStringOnOff(newValue)), ElytraOptions.getInstance());
        }
        else if(args == null || args.length <= 0) // no arguments specified.
        {
            ...
            ep.toNinOnlinePlayer().sendMessage(MessageColor.INFO, String.format("Flight boosting is now turned %s", BooleanUtils.toStringOnOff(ep.isUsingBoost())), ElytraOptions.getInstance());
        }
    }
}
